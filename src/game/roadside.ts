...
export type RoadsideType = 'tree' | 'lamp' | 'sign'
...
export function getRoadsideObjects(fromDist: number, toDist: number): RoadsideObject[] {
  ...
  for (let i = treeStart; i <= treeEnd; i++) {
    ...
    result.push({ distM: d, side, type: 'tree', offset: 0.12 + hash(i * 19) * 0.2 })
    // Sometimes tree on both sides
    if (hash(i * 23) < 0.3) {
      result.push({ distM: d + 5, side: (side * -1) as -1 | 1, type: 'tree', offset: 0.15 + hash(i * 29) * 0.15 })
    }
  }
  ...
  for (let i = signStart; i <= signEnd; i++) {
    ...
    if (d < fromDist || d > toDist) continue
    result.push({ distM: d, side: hash(i * 41) < 0.5 ? -1 : 1, type: 'sign', offset: 0.06 })
  }
