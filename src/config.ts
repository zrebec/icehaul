...
/**
 * Minimum straight, in metres, that must lie ahead of the point where a
 * non-asphalt segment begins. Surfaces and curves are generated by two separate
 * chains, so before this nothing stopped ice from starting inside the sharpest
 * bend — a combination the player cannot read in advance and, until the lateral
 * model was fixed, could not hold at any speed.
 *
 * Must exceed {@link TURN_RAMP_M} (60). At the 50 m originally floated, the truck
 * would already be inside `rampIn` by the time the surface changed, which is the
 * exact situation this is meant to prevent.
 */
 ...
 * pathological seed cannot spin the generation loop forever.
 */
export const SAFE_ENTRY_SEARCH_LIMIT_M = 1500
